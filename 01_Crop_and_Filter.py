...
def main():
    """
    Main function to run the loading, cropping, and filtering pipeline.
    """
    # --- Configuration ---
    ...
    # Define the duration you want to keep (in seconds)
    crop_duration = 20.0
    
    # Define the output filename for the cropped and filtered data
    output_fname = os.path.join(output_dir, f"{base_name}_cropped_filtered_raw.fif")
    ...
    # --- Step 2: Crop the Data ---
    print(f"\n--- Cropping data to the first {crop_duration} seconds ---")
    raw.crop(tmin=0, tmax=crop_duration)
    print("Cropping complete.")

    # --- Step 3: Filter the Cropped Data ---
    ...
    print("\nPlotting Power Spectral Density of the cropped segment.")
    
    fig_before = raw.compute_psd(fmax=80).plot(show=False)
    fig_before.suptitle('Before Filtering (Cropped)', y=0.95)

    fig_after = raw_filtered.compute_psd(fmax=80).plot(show=False)
    fig_after.suptitle('After Filtering (Cropped)', y=0.95)
    
    plt.show(block=True) # Show both PSD plots

    # --- Step 5: Save the Cropped and Filtered Data ---
    print(f"\nSaving cropped and filtered data to: {output_fname}")
    raw_filtered.save(output_fname, overwrite=True)
    print("\nDone. You can now use this shorter file to test the next preprocessing steps.")
